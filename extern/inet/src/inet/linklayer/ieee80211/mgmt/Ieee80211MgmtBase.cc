...
int n_messages_link_layer = 0;//DEBUG_ENIO
...
{
    //n_messages_link_layer++;
    //double send_time = simTime().dbl();	
    //std::cout  << "Ieee80211MgmtBase::handleMessage: " << n_messages_link_layer << std::endl;//DEBUG_ENIO
    if (!isOperational){
        //std::cout  << "Ieee80211MgmtBase::handleMessage: (1)" << std::endl;//DEBUG_ENIO
        throw cRuntimeError("Message '%s' received when module is OFF", msg->getName());
    }
    if (msg->isSelfMessage()) {
        // process timers
        EV << "Timer expired: " << msg << "\n";
        std::cout  << "Ieee80211MgmtBase::handleMessage: (2)" << std::endl;//DEBUG_ENIO
        handleTimer(msg);
    }
    else if (msg->arrivedOn("macIn")) {
        // process incoming frame
        //std::cout  << "Ieee80211MgmtBase::handleMessage: (3)" << std::endl;//DEBUG_ENIO
        EV << "Frame arrived from MAC: " << msg << "\n";
        Ieee80211DataOrMgmtFrame *frame = check_and_cast<Ieee80211DataOrMgmtFrame *>(msg);
        processFrame(frame);
    }
    else if (msg->arrivedOn("agentIn")) {
        //std::cout  << "Ieee80211MgmtBase::handleMessage: (4)" << std::endl;//DEBUG_ENIO
        ...
        //std::cout  << "Ieee80211MgmtBase::handleMessage: Packet arrived from upper layers: " << pk << "," << send_time << std::endl;//DEBUG_ENIO
