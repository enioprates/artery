...
double num_message_contention = 0;     //DEBUG_ENIO
...
void Contention::handleWithFSM(EventType event)
{
    ...
    FSMA_Switch(fsm) {
        FSMA_State(IDLE) {
            //std::cout  << "contention: STAGE1 0" << std::endl;//DEBUG_ENIO
            FSMA_Enter(mac->sendDownPendingRadioConfigMsg());
            //std::cout  << "contention: STAGE1 1" <<std::endl;//DEBUG_ENIO
            FSMA_Event_Transition(Starting-IFS-and-Backoff,
                    event == START && mediumFree,
                    IFS_AND_BACKOFF,
                    scheduleTransmissionRequest();
                    );
            //std::cout  << "contention: STAGE1 2" <<std::endl;//DEBUG_ENIO        
            FSMA_Event_Transition(Busy,
                    event == START && !mediumFree,
                    DEFER,
                    ola();//DEBUG_ENIO 
                    );
            //std::cout  << "contention: STAGE1 3" <<std::endl;//DEBUG_ENIO        
            FSMA_Ignore_Event(event==MEDIUM_STATE_CHANGED);
            //std::cout  << "contention: STAGE1 4 " <<std::endl;//DEBUG_ENIO
            FSMA_Ignore_Event(event==CORRUPTED_FRAME_RECEIVED);
            //std::cout  << "contention: STAGE1 5" <<std::endl;//DEBUG_ENIO
            FSMA_Fail_On_Unhandled_Event();
        }
        FSMA_State(DEFER) {
            //std::cout  << "contention: STAGE2" <<std::endl;//DEBUG_ENIO
            FSMA_Enter(mac->sendDownPendingRadioConfigMsg());
        ...
        FSMA_State(IFS_AND_BACKOFF) {
            //std::cout  << "contention: STAGE3" <<std::endl;//DEBUG_ENIO
            FSMA_Enter();
    ...
    //std::cout  << "contention: STAGE1 6" <<std::endl;//DEBUG_ENIO
    emit(stateChangedSignal, fsm.getState());
...
void Contention::ola()
{
//std::cout  << "Contention::ola()- DEFER" <<std::endl;//DEBUG_ENIO
}

void Contention::scheduleTransmissionRequest()
{
    num_message_contention++; //DEBUG_ENIO
    //std::cout  << "Contention::scheduleTransmissionRequest: "<< num_message_contention <<std::endl;//DEBUG_ENIO


    std::ofstream myfile_send;
    ASSERT(mediumFree);
    ...
    //myfile_send << "B-" << backoffSlots << "," << waitInterval << ","
    //                << slotTime << "," << num_message_contention <<std::endl;
    ...
    //(ENIO)(JOAO)
    myfile_send.open("BACKOFFS_count.csv", std::ofstream::out );
    //myfile_send << "B-" << backoffSlots << "," << waitInterval << ","
    //                << slotTime << "," << num_message_contention <<std::endl;
    myfile_send << num_message_contention <<std::endl;
    myfile_send.close();
    //(ENIO)(JOAO)
