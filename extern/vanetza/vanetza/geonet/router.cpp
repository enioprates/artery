...
int num_send_msg_router = 0;			//DEBUG_ENIO
...
{
    num_send_msg_router++;  //DEBUG_ENIO
    DataConfirm result;
    //std::cout  << "Router:request : "<< num_send_msg_router <<std::endl;//DEBUG_ENIO
    ...
    if (result.accepted()) {
        ...
        auto transmit = [this, ctrl](PendingPacket::Packet&& packet) {
            ...
            //std::cout  << "Router::request:Payload(L): "<< payload->size() <<std::endl;//DEBUG_ENIO
            ...
            if (m_mib.itsGnSecurity) {
                ...
                //std::cout  << "Router::request:Payload(K): "<< payload->size() <<std::endl;//DEBUG_ENIO
            }
...
{
    assert(packet);
    //std::cout  << "Router: 3" <<std::endl;//DEBUG_ENIO
...
{
    //std::cout  << "Router:pass_down_2 : "<< num_send_msg_router <<std::endl;//DEBUG_ENIO
    //std::cout  << "Router::pass_down:Payload (1): "<< payload->size() <<std::endl;//DEBUG_ENIO
    //std::cout  << "Router::pass_down:PDU (1): " <<std::endl;//DEBUG_ENIO

   	//double send_time = (double) m_runtime.now();													//enviando dados de tempo da msg em seg

    assert(pdu);
    assert(payload);
    if (pdu->secured()) {
        //std::cout  << "Router:pass_down_2: PDU_SECURED" <<" , " << send_time <<std::endl;//DEBUG_ENIO
        //std::cout  << "Router:pass_down_2: PDU_SECURED" <<std::endl;//DEBUG_ENIO
