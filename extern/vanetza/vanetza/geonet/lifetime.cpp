...
bool Lifetime::operator==(const Lifetime& other) const
{
    ...
    const auto min_value = 0.010 * units::si::seconds; //ENIO_DEBUG
...
void Lifetime::encode(units::Duration duration)
{
    ...
    if (seconds >= 630.0) {
        ...
        set(Base::_50_MS, std::lround(seconds / 0.010)); //ENIO_DEBUG
    }
}
