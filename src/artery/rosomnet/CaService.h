...
class CaService : public ItsG5BaseService
{
	...
	private:
		long numSent;					//Contador de mensagens Enviadas
        long numReceived;				//Contador de mensagens recebidas		
		...
		typedef Timer Timerosomnet; //rosomnet compliancy

		const VehicleDataProvider* mVehicleDataProvider;
		const Timerosomnet* mTimer; //rosomnet compliancy
		//const Timer* mTimer;
		...
		vanetza::units::Velocity mLastRawmSpeed;
		vanetza::units::Angle mLastRawHeading;
		omnetpp::SimTime mLastRawTimestamp;
		omnetpp::SimTime mLastLowRawTimestamp;
		
		omnetpp::SimTime deltattargetMin;
		omnetpp::SimTime deltattargetMax;
		omnetpp::SimTime deltattarget;
		double TVX;
		double TVY;
		double distanceleader;
};
vanetza::asn1::Cam createCooperativeAwarenessMessage(const VehicleDataProvider&, uint16_t genDeltaTime);
vanetza::asn1::Raw createRawMessage(const VehicleDataProvider&, uint16_t genDeltaTime);
