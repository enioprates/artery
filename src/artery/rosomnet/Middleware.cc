...
#include "artery/rosomnet/ItsG5PromiscuousService.h"
...
int num_send_msg_middleware_2 = 0;			//DEBUG_ENIO
int num_send_msg_middleware_1 = 0;			//DEBUG_ENIO
		...
		std::unique_ptr<vanetza::geonet::DownPacket> payload)
{
	...
	//num_send_msg_middleware_1++;
	//double send_time = simTime().dbl();												//enviando dados de tempo da msg em seg
	//std::cout  << "Middleware::request (1): "<< num_send_msg_middleware_1 << " , " << send_time <<std::endl;//DEBUG_ENIO
...
{
	num_send_msg_middleware_2++;
	double send_time = simTime().dbl();												//enviando dados de tempo da msg em seg
	//std::cout  << "Middleware::request (2): " << num_send_msg_middleware_2 << " , " << send_time << std::endl;//DEBUG_ENIO
	...
	switch (req.gn.transport_type) {
		case geonet::TransportType::SHB: {
			...
			//std::cout  << "Middleware::request::mGeoRouter->request (B1)" << " , " << send_time << std::endl;//DEBUG_ENIO
			confirm = mGeoRouter->request(request, std::move(payload));
			//std::cout  << "Middleware::request::mGeoRouter->request (B2)" << " , " << send_time << std::endl;//DEBUG_ENIO
		}
			break;
	...
	scheduleRuntime();
	//double send_time_3 = simTime().dbl();												//enviando dados de tempo da msg em seg
	//std::cout  << "Middleware::request (3): " << num_send_msg_middleware_2 << " , " << send_time_3 << std::endl;//DEBUG_ENIO
...
{
	updateServices();
	scheduleAt(simTime() + 0.01, mUpdateMessage);//mUpdateInterval PlatoonMode change
}
...
{
	//double send_time = simTime().dbl();												//enviando dados de tempo da msg em seg
	//std::cout  << "Middleware::scheduleRuntime (1)" << " , " << send_time << std::endl;//DEBUG_ENIO
	...
	//double send_time_2 = simTime().dbl();												//enviando dados de tempo da msg em seg
	//std::cout  << "Middleware::scheduleRuntime (1)" << " , " << send_time_2 << std::endl;//DEBUG_ENIO
